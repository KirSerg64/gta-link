...
def replace_tracklet_keys(tracklets, mapping):
    ...
    for old_id in old_mapped_ids:
        ...
        if (new_id != old_id) and (old_id not in old_new_intersection):
            old_tracklet = copy.deepcopy(tracklets[new_id])
...
def main():
    ...
    seqs_tracks = [seq for seq in os.listdir(seq_tracks_dir) if seq.endswith('.pkl')]

    seqs_tracks.sort()

    tracklets_spartial = {0:None, 1:None}  # always only previous and current track
    for seq_idx, seq in enumerate(seqs_tracks):
        ...
        if seq_idx == 0:
            tracklets_spartial[0] = {'tracklets': tmp_trklets, 
                                    "time_delta":max([max(tracklet.times) for _,tracklet in tmp_trklets.items()])}
            continue
        else:
            tracklets_spartial[1] = {'tracklets': tmp_trklets, 
                                    "time_delta":max([max(tracklet.times) for _,tracklet in tmp_trklets.items()])}
        ...
        logger.info(f"Unique start tracklets: {len(list(mapping.keys()))} Unique end tracklets: {len(list(mapping.values()))}")
        ...
        tracklets_spartial[0] = new_tracklets_dict
