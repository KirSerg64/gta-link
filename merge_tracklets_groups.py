...
def merge_tracklets(tracklets, seq2Dist, Dist, seq_name=None, max_x_range=None, max_y_range=None, merge_dist_thres=None):
    ...
    # display_Dist(seq2Dist, seq_name, isMerged=False, isSplit=True)         # used to display Dist, debug line, delete later=
    ...
    while (np.any(Dist[non_diagonal_mask] < merge_dist_thres)):
        ...
        # print(f"{Dist[19,15]}  > {merge_dist_thres}")
...
# Wrong version
# def merge_tracklets(tid2track, seq2Dist, Dist, seq_name=None, max_x_range=None, max_y_range=None, merge_dist_thres=0.4):
#     """
#     Merges tracklets within a single video based on distance and spatial constraints.

#     Args:
#         tid2track (dict): Dictionary mapping track IDs to Tracklet objects.
#         seq2Dist (dict): Dictionary of distance matrices for each sequence.
#         Dist (ndarray): Distance matrix for the current sequence.
#         seq_name (str): Name of the sequence.
#         max_x_range (float): Maximum allowed x distance for spatial continuity.
#         max_y_range (float): Maximum allowed y distance for spatial continuity.
#         merge_dist_thres (float): Cosine distance threshold for merging.

#     Returns:
#         dict: Merged tracklets (key: new track id, value: Tracklet).
#     """
#     # Original function body unchanged
#     merged_tracklets = {}
#     used = set()
#     tids = list(tid2track.keys())
#     next_id = 0
...
#         t1 = tid2track[tid1]
#         merged = Tracklet()
#         merged.track_id = next_id
#         merged.times = t1.times.copy()
#         merged.bboxes = t1.bboxes.copy()
#         merged.features = t1.features.copy()
#         if hasattr(t1, "scores"):
#             merged.scores = t1.scores.copy()
#         used.add(tid1)

#         for j, tid2 in enumerate(tids):
#             if tid2 in used or tid1 == tid2:
#                 continue
#             t2 = tid2track[tid2]
#             # Only consider merging if t2 starts after t1 ends
#             if max(t1.times) < min(t2.times):
#                 spatial_ok = True
#                 if max_x_range is not None and max_y_range is not None:
#                     spatial_ok = check_spatial_constraints(t1, t2, max_x_range, max_y_range)
#                 if spatial_ok and Dist[i, j] < merge_dist_thres:
#                     merged.times += t2.times
#                     merged.bboxes += t2.bboxes
#                     merged.features += t2.features
#                     if hasattr(t2, "scores"):
#                         merged.scores += t2.scores
#                     used.add(tid2)
...
def merge_tracklets_across_videos(tracklets1, tracklets2, merge_dist_thres=0.4, max_x_range=None, max_y_range=None):
    """
    Merges tracklets from two consecutive videos, associating tracklets of the same person across the video boundary.

    Args:
        tracklets1 (dict): Tracklets from the first video (key: track id, value: Tracklet).
        tracklets2 (dict): Tracklets from the second video (key: track id, value: Tracklet).
        merge_dist_thres (float): Cosine distance threshold for merging.
        max_x_range (float): Maximum allowed x distance for spatial continuity.
        max_y_range (float): Maximum allowed y distance for spatial continuity.
    ...
    # Build distance matrix between all tracklets1 and tracklets2
    ids1 = list(tracklets1.keys())
    ids2 = list(tracklets2.keys())
    Dist = np.ones((len(ids1), len(ids2)))  # initialize with max distance
    ...
    for i, tid1 in enumerate(ids1):
        for j, tid2 in enumerate(ids2):
            ...
            t2 = tracklets2[tid2]
            # Only consider merging if t2 starts after t1 ends
            #TO-DO add to t2.times the length of first video to ensure that t2 starts after t1 ends
            t2.times = [t + first_video_length for t in t2.times]
            if max(t1.times) < min(t2.times):
                # Optional: check spatial continuity
                spatial_ok = True
                if max_x_range is not None and max_y_range is not None:
                    spatial_ok = check_spatial_constraints(t1, t2, max_x_range, max_y_range)
                if spatial_ok:
                    Dist[i, j] = get_distance(tid1, tid2, t1, t2)

    # Greedy matching: for each tracklet1, find best match in tracklets2 under threshold
    matched2 = {}
    for i, tid1 in enumerate(ids1):
        j_best = np.argmin(Dist[i])
        if Dist[i, j_best] < merge_dist_thres:
            tid2 = ids2[j_best]
            if tid2 not in used2:
                # Merge t1 and t2
                t1 = tracklets1[tid1]
                t2 = tracklets2[tid2]
                merged = Tracklet()
                merged.track_id = next_id
                merged.times = t1.times + t2.times
                merged.bboxes = t1.bboxes + t2.bboxes
                merged.features = t1.features + t2.features
                if hasattr(t1, "scores") and hasattr(t2, "scores"):
                    merged.scores = t1.scores + t2.scores
                merged_tracklets[next_id] = merged
                used2.add(tid2)
                matched2[tid2] = next_id
                next_id += 1
            else:
                # Already matched, skip
                continue
        else:
            # No match, keep t1 as is
            t1 = tracklets1[tid1]
            merged = Tracklet()
            merged.track_id = next_id
            merged.times = t1.times
            merged.bboxes = t1.bboxes
            merged.features = t1.features
            if hasattr(t1, "scores"):
                merged.scores = t1.scores
            merged_tracklets[next_id] = merged
            next_id += 1

    # Add unmatched tracklets from tracklets2
    for tid2 in ids2:
        if tid2 not in used2:
            t2 = tracklets2[tid2]
            merged = Tracklet()
...
def main():
    ...
    for seq_idx, seq in enumerate(seqs_tracks):
        ...
        tracklets_spartial[seq_idx] = {'tracklets': tmp_trklets, 
        ...
        #     splitTracklets = split_tracklets(tmp_trklets1, eps=args.eps, max_k=args.max_k, min_samples=args.min_samples, len_thres=args.min_len)
        # else:
        #     splitTracklets = tmp_trklets1
        ...
        # os.makedirs(os.path.join(save_path, "processed"), exist_ok=True)
        # new_sct_output_path = os.path.join(save_path, "processed", '{}.txt'.format(seq_name))
